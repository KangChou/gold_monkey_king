...
def buildCNN(w, h, c):
    ...
    with tf.compat.v1.name_scope('input'):
        x = tf.compat.v1.placeholder(tf.float32, shape=[None, w, h, c], name='x-input')
        y_ = tf.compat.v1.placeholder(tf.int32, shape=[None, ], name='y_')

    # 第一个卷积层 + 池化层（100——>50)
    with tf.compat.v1.name_scope('conv2d-pool-1'):
    ...
    # 第二个卷积层 + 池化层 (50->25)
    with tf.compat.v1.name_scope('conv2d-pool-2'):
    ...
    # 第三个卷积层 + 池化层 (25->12)
    with tf.compat.v1.name_scope('conv2d-pool-3'):
    ...
    # 第四个卷积层 + 池化层 (12->6)
    with tf.compat.v1.name_scope('conv2d-pool-4'):
    ...
    # 全连接层
    with tf.compat.v1.name_scope('layer-1'):
        dense1 = tf.compat.v1.layers.dense(inputs=re1,
                                 units=1024,
                                 activation=tf.nn.relu,
                                 kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                                 kernel_regularizer=tf.keras.regularizers.l2(0.5 * (0.003)))
    with tf.compat.v1.name_scope('layer-2'):
        dense2 = tf.compat.v1.layers.dense(inputs=dense1,
                                 units=512,
                                 activation=tf.nn.relu,
                                 kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                                 kernel_regularizer=tf.keras.regularizers.l2(0.5 * (0.003)))
    with tf.compat.v1.name_scope('layer-3'):
        dense3 = tf.compat.v1.layers.dense(inputs=dense2,
                                 units=32,
                                 activation=None,
                                 kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                                 kernel_regularizer=tf.keras.regularizers.l2(0.5 * (0.003)))
    with tf.compat.v1.name_scope('layer-4'):
        logits = tf.compat.v1.layers.dense(inputs=dense3,
                                 units=7,
                                 activation=None,
                                 kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                                 kernel_regularizer=tf.keras.regularizers.l2(0.5 * (0.003)))
...
def accCNN(logits, y_):
    with tf.compat.v1.name_scope('cross_entropy'):
        loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(labels=y_, logits=logits)
    with tf.compat.v1.name_scope('AdamOptimizer'):
        train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(loss)   #优化器
...
def runable(x_train, y_train, train_op, loss, acc, x, y_, x_val, y_val):
    ...
    n_epoch = 50              #之前是100轮，有点多，改成增量训练模型后就可以多轮次增加训练了
    batch_size = 30             #20是每批读取数据的数量

    ###创建saver对象用于模型保存和加载
    saver = tf.compat.v1.train.Saver()

    sess = tf.compat.v1.InteractiveSession()                          #手动启动session对话，还需关闭（也可使用with）
    sess.run(tf.compat.v1.global_variables_initializer())             #对全局变量初始化先
